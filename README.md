...
> **Before you start:**
> - **Required** — a Google OAuth Desktop client (Sheets + Drive + Slides scopes) to authenticate against your Google account. Set up during `/gantt:init` (Gate 5 walks you through Cloud Console), or pre-create it from [console.cloud.google.com](https://console.cloud.google.com) → APIs & Services → Credentials → OAuth client ID → Desktop app, and drop `credentials.json` into `~/.config/gantt/` first. See the **Google OAuth setup** section below for full details.
> - **Required** — Python 3.10+.
> - **Optional** — an Anthropic API key in `~/.config/gantt/.env` to enable LLM-generated section summary slides in `gantt deck`. Decks still generate without it; the summary slide is simply skipped. See the **Anthropic API key** section below for setup.
...
<details>
<summary><b>Google OAuth setup (Required, once per install)</b></summary>

Needed because the skill's `bootstrap` step writes to a Google Sheet on your behalf, and `gantt deck` writes to Google Slides + Drive.
...
<details>
<summary><b>Anthropic API key for deck narrative summaries (Optional)</b></summary>

Enables an LLM-generated paragraph summary slide on each `gantt deck`
section divider. **Decks generate fine without it** — the narrative slide
is simply skipped, and the result line reports `narrative: skipped (no key)`.

What it enables:

- ~3-sentence audience-targeted summary slide per deck section (tactical → engineering ICs, strategic → engineering leadership).
- Generated by Claude on the Anthropic API. Default model is **Claude Haiku 4.5** (cheapest, fastest). Override per-invocation: `gantt deck --model sonnet` or `--model opus`.

Setup:

1. Get a key from [console.anthropic.com](https://console.anthropic.com) → Settings → API Keys → Create Key.
2. Drop it in `~/.config/gantt/.env`:
   ```bash
   mkdir -p ~/.config/gantt
   echo 'ANTHROPIC_API_KEY=sk-ant-...' >> ~/.config/gantt/.env
   ```
3. The next `gantt deck` invocation picks it up automatically — the CLI loads `~/.config/gantt/.env` on every start (file format: `KEY=value`, `#` comments, optional `export` prefix).

Overrides + opt-out:

- `GANTT_ENV_FILE=/path/to/other.env` — read from a different file.
- A shell-set `ANTHROPIC_API_KEY=...` takes precedence over the file value (`.env` never clobbers an already-set var).
- `gantt deck --no-narrative` — opt out per invocation even when a key is set.

Rough cost per deck section (highly approximate; varies with payload size):

- **Haiku 4.5** (default): well under $0.01
- **Sonnet 4.6**: ~$0.01
- **Opus 4.7**: ~$0.03

A typical deck has 4-6 sections, so one `gantt deck` call on Haiku is fractions of a cent. Opus is still measured in cents per call. See [anthropic.com/pricing](https://www.anthropic.com/pricing) for current rates.
...
**LLM section summaries (optional)**: by default each section divider
gets a Claude-generated paragraph summary slide right after it. Requires
`ANTHROPIC_API_KEY` in `~/.config/gantt/.env` — see the **Anthropic API
key** collapsible in Quick Start above. Without a key the summary slide
is silently skipped (everything else still works). Opt out per
invocation with `--no-narrative`, or override the model with
`--model sonnet` / `--model opus` (default `haiku`).
